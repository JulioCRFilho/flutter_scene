...
export '../inspector/schema_property_row.dart' show EnumRow;
...
class _ComponentEditor extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget row(ComponentPropertyDef def) => SchemaPropertyRow(
      componentType: type,
      def: def,
      ...
      children: [
        ...
        for (final entry in extras)
          if (mixedFor(entry.key, null))
            ReadOnlyRow(label: entry.key, text: 'Mixed values')
          else
            PropertyValueRow(
              label: entry.key,
              value: entry.value,
